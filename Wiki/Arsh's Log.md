...
<b>As of week 8</b>, we have completed the game development and are performing bug fixes. We constantly kept in touch this week to keep the status visible to everyone. As we tested each other's code, communication is required to find and fix bugs. We also did the UML diagrams this week which required communiction to understand the pattern implementation. We kept our co-ordination by keeping communication channel alive and closed most the bugs with success. We kept pushing the changes by communicating on slack and maintained the code in shippable state.
